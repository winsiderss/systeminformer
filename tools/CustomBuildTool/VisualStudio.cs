...
namespace CustomBuildTool
{
    public static unsafe class VisualStudio
    {
                        ...
                        ).Succeeded)
                    {
                            ...
                            ).Succeeded)
                        {
                                ...
                                ).Succeeded)
                            {
                                ...
                                while (true)
                                {
                                    enumSetupInstances->Next(EnumSetupInstancesInterfacePtr, 1, &SetupInstancePtr, &instancesFetched);
        ...
        internal static VisualStudioInstance GetVisualStudioInstance()
        {
            if (VisualStudioInstance == null && VisualStudioInstanceList != null)
            {
                //if (VisualStudioInstanceList.Any(t => t.Name.Contains("preview", StringComparison.OrdinalIgnoreCase)))
                //{
                //    return VisualStudioInstanceList.Last(); // stable
                //}

                VisualStudioInstance = VisualStudioInstanceList.First();
            }
    ...
    internal readonly unsafe struct ISetupHelperVTable
    {
        ...
        // ISetupHelper
        public readonly delegate* unmanaged[Stdcall]<IntPtr, IntPtr, uint*, HRESULT> ParseVersion;
        public readonly delegate* unmanaged[Stdcall]<IntPtr, IntPtr, uint*, uint*, HRESULT> ParseVersionRange;
    }
    ...
    internal readonly unsafe struct ISetupConfigurationVTable
    {
        ...
        public readonly delegate* unmanaged[Stdcall]<IntPtr, IntPtr*, HRESULT> EnumInstances;
        // STDMETHOD(GetInstanceForCurrentProcess)(_Out_ ISetupInstance ** ppInstance) = 0;
        public readonly delegate* unmanaged[Stdcall]<IntPtr, IntPtr*, HRESULT> GetInstanceForCurrentProcess;
        // STDMETHOD(GetInstanceForPath)(_In_z_ LPCWSTR wzPath, _Out_ ISetupInstance **ppInstance) = 0;
        public readonly delegate* unmanaged[Stdcall]<IntPtr, IntPtr, IntPtr*, HRESULT> GetInstanceForPath;
    }
