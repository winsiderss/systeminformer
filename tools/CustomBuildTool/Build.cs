...
namespace CustomBuildTool
{
    public static class Build
    {
        ...
        public static unsafe bool BuildSdk(BuildFlags Flags)
        {
            bool verbose = Flags.HasFlag(BuildFlags.BuildVerbose);
            ...
            {
                if (Flags.HasFlag(BuildFlags.Build32bit))
                    Win32.CopyIfNewer("bin\\Debug32\\SystemInformer.lib", "sdk\\lib\\i386\\SystemInformer.lib", verbose);
                if (Flags.HasFlag(BuildFlags.Build64bit))
                    Win32.CopyIfNewer("bin\\Debug64\\SystemInformer.lib", "sdk\\lib\\amd64\\SystemInformer.lib", verbose);
                if (Flags.HasFlag(BuildFlags.BuildArm64bit))
                    Win32.CopyIfNewer("bin\\DebugARM64\\SystemInformer.lib", "sdk\\lib\\arm64\\SystemInformer.lib", verbose);
            }

            if (Flags.HasFlag(BuildFlags.BuildRelease))
            {
                if (Flags.HasFlag(BuildFlags.Build32bit))
                    Win32.CopyIfNewer("bin\\Release32\\SystemInformer.lib", "sdk\\lib\\i386\\SystemInformer.lib", verbose);
                if (Flags.HasFlag(BuildFlags.Build64bit))
                    Win32.CopyIfNewer("bin\\Release64\\SystemInformer.lib", "sdk\\lib\\amd64\\SystemInformer.lib", verbose);
                if (Flags.HasFlag(BuildFlags.BuildArm64bit))
                    Win32.CopyIfNewer("bin\\ReleaseARM64\\SystemInformer.lib", "sdk\\lib\\arm64\\SystemInformer.lib", verbose);
            }
            ...
            Win32.CopyIfNewer("SystemInformer\\include\\phappres.h", "sdk\\include\\phappres.h", verbose);
            Win32.CopyIfNewer("SystemInformer\\sdk\\phapppub.h", "sdk\\include\\phapppub.h", verbose);
            Win32.CopyIfNewer("SystemInformer\\sdk\\phdk.h", "sdk\\include\\phdk.h", verbose);
        ...
        public static bool BuildDeployUpdateConfig()
        {
            ...
            try
            {
                ...
                using (HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Post, buildPostSfUrl))
                {
                    ...
                    var httpResult = Http.SendMessage(requestMessage);
...
//;
//; Do not link exports at runtime.
//;
//";
            StringBuilder content = new StringBuilder();
            string[] exports = File.ReadAllLines("SystemInformer\\SystemInformer.def");
            int total = 0;
            ...
            if (ReleaseBuild)
            {
                ...
                {
                    var value = Random.Shared.Next(1000, 1000 + total); // ushort.MaxValue
            ...
            foreach (string line in exports)
            {
                ...
                {
                    int length = span.IndexOf("@", StringComparison.OrdinalIgnoreCase);

                    if (length != -1)
                    {
                        var ordinal = ordinals[0]; ordinals.RemoveAt(0);

                        content.Append(span.Slice(0, length));
                        content.Append('@');
                        content.Append(ordinal);
                        content.Append(" NONAME");
            ...
            string export_content = content.ToString().TrimEnd();
