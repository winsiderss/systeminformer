...
/**
 * \brief Retrieves the modules loaded into the current process.
 *
 * \param[in] Process The process to get the modules of.
 * \param[out] Modules A variable which receives a pointer to a structure
 * containing information about the process modules. The structure must be
 * freed with KphFreeProcessModules.
 *
 * \return Successful or errant status.
 */
_IRQL_requires_max_(PASSIVE_LEVEL)
    ...
    )
{
    ...
    *Modules = NULL;
    ...
    //
    // Suspend the process to prevent the loader from changing the links on us.
    //
    ...
    if (processInfo.IsSubsystemProcess)
    {
    ...
    {
#if _WIN64
        if (processInfo.IsWow64Process)
        {
            PPEB32 peb;
            PPEB_LDR_DATA32 ldr;

            peb = (PPEB32)processInfo.BasicInfo.PebBaseAddress;
            ldr = (PPEB_LDR_DATA32)peb->Ldr;

            for (PLIST_ENTRY32 link = (PLIST_ENTRY32)ldr->InLoadOrderModuleList.Flink;
                 link != &ldr->InLoadOrderModuleList;
                 link = (PLIST_ENTRY32)link->Flink)
            {
                numberOfModules++;

                if (numberOfModules >= KPH_MODULES_MAX_COUNT)
                {
                    numberOfModules = 0;
                    break;
                }
...
#endif
        {
            ...
            peb = (PPEB)processInfo.BasicInfo.PebBaseAddress;
            ldr = peb->Ldr;

            for (PLIST_ENTRY link = ldr->InLoadOrderModuleList.Flink;
                 link != &ldr->InLoadOrderModuleList;
                 link = link->Flink)
            {
                numberOfModules++;
    ...
    size = (sizeof(RTL_PROCESS_MODULES)
            + (sizeof(RTL_PROCESS_MODULE_INFORMATION)
               * (numberOfModules - 1)));

    modules = KphAllocatePaged(size, KPH_TAG_MODULES);
    if (!modules)
    {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto Exit;
    ...
    __try
    {
        ...
        if (processInfo.IsWow64Process)
        {
                 ...
                 link = (PLIST_ENTRY32)link->Flink)
            {
                ...
                NT_ASSERT(modules->NumberOfModules < numberOfModules);

                info = &modules->Modules[modules->NumberOfModules];
                entry = CONTAINING_RECORD(link, LDR_DATA_TABLE_ENTRY32, InLoadOrderLinks);

                info->ImageBase = (PVOID)entry->DllBase;
                info->ImageSize = entry->SizeOfImage;
                info->Flags = entry->Flags;
                info->InitOrderIndex = (USHORT)modules->NumberOfModules;

                modules->NumberOfModules++;
                if (modules->NumberOfModules >= numberOfModules)
                {
                    break;
                }
            }
...
/**
 * \brief Frees process modules info previously retrieved by KphGetProcessModules.
 *
 * \param[in] Modules The modules info to free.
 */
_IRQL_requires_max_(PASSIVE_LEVEL)
