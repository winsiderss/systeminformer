...
static VOID PhPluginWindowNotifyEventCallback(
    _In_opt_ PVOID Parameter,
    _In_opt_ PVOID Context
    )
{
    PPH_PLUGIN_WINDOW_NOTIFY_EVENT Event = Parameter;
    PPH_PLUGIN_WINDOW_CALLBACK_REGISTRATION callback = Context;

    switch (Event->Type)
    {
    case PH_PLUGIN_WINDOW_EVENT_TYPE_TOPMOST:
        {
            if (callback->Type != PH_PLUGIN_WINDOW_EVENT_TYPE_TOPMOST)
                break;

            PhSetWindowAlwaysOnTop(callback->WindowHandle, Event->TopMost);
        }
        break;
    case PH_PLUGIN_WINDOW_EVENT_TYPE_FONT:
        {
            if (callback->Type != PH_PLUGIN_WINDOW_EVENT_TYPE_FONT)
                break;

            SendMessage(callback->WindowHandle, WM_SETFONT, (WPARAM)Event->FontHandle, TRUE);
        }
        break;
    }
}

PPH_PLUGIN_WINDOW_CALLBACK_REGISTRATION PhRegisterWindowNotifyEvents(
    ...
    )
{
    ...
    callback = PhAllocateZero(sizeof(PH_PLUGIN_WINDOW_CALLBACK_REGISTRATION));
    callback->WindowHandle = WindowHandle;
    callback->Type = Type;
    ...
    PhRegisterCallback(
        ...
        PhPluginWindowNotifyEventCallback, 
        callback, 
...
VOID PhUnregisterWindowNotifyEvents(
    _In_ HWND WindowHandle,
    _In_ PPH_PLUGIN_WINDOW_CALLBACK_REGISTRATION Callback
    )
{
    PhUnregisterCallback(PhGetGeneralCallback(GeneralCallbackWindowNotifyEvent), &Callback->Registration);
    PhFree(Callback);
}
    ...
    )
{
    ...
    event = PhAllocateZero(sizeof(PH_PLUGIN_WINDOW_NOTIFY_EVENT));
    event->Type = PH_PLUGIN_WINDOW_EVENT_TYPE_TOPMOST;
    event->TopMost = TopMost;

    PhInvokeCallback(PhGetGeneralCallback(GeneralCallbackWindowNotifyEvent), event);
    ...
    )
{
    ...
    event = PhAllocateZero(sizeof(PH_PLUGIN_WINDOW_NOTIFY_EVENT));
    event->Type = PH_PLUGIN_WINDOW_EVENT_TYPE_FONT;
    event->FontHandle = FontHandle;
