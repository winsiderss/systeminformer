    ...
    )
{
    if (_InterlockedDecrement(&Context->RefCount) == 0)
    {
        ...
        if (Context->ImageAddressList)
        {
            ...
            PhDereferenceObject(Context->ImageAddressList);
        }
        ...
        if (Context->PageTamperingList)
        {
            ...
            PhDereferenceObject(Context->PageTamperingList);
        }
    ...
    case WM_INITDIALOG:
        {
            ...
            if (PhGetIntegerPairSetting(L"MemoryModifiedWindowPosition").X)
                PhLoadWindowPlacementFromSetting(L"MemoryModifiedWindowPosition", L"MemoryModifiedWindowSize", hwndDlg);
            else
...

            {
                ...
                if (!NT_SUCCESS(status))
                {
                    PhShowStatus(GetParent(hwndDlg), L"", status, 0);
    ...
    case WM_DESTROY:
        {
            ...
            PhSaveWindowPlacementToSetting(L"MemoryModifiedWindowPosition", L"MemoryModifiedWindowSize", hwndDlg);
