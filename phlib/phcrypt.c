    ...
    )
{
    ...
    error = SymCryptCcmValidateParameters(SymCryptAesBlockCipher, NonceLength, AuthDataLength, DataLength, TagLength);
    if (error == SYMCRYPT_NO_ERROR)
    {
        SymCryptCcmEncrypt(
            SymCryptAesBlockCipher,
    ...
    )
{
    ...
    *StateHandle = NULL;
    ...
    error = SymCryptGcmExpandKey(&state->ExpandedKey, SymCryptAesBlockCipher, (PCBYTE)Key, KeyLength);
    if (error != SYMCRYPT_NO_ERROR)
    {
        PhFree(state);
        return PhSymCryptErrorToStatus(error);
    ...
    )
{
    ...
    if (Encrypt)
        SymCryptCfbEncrypt(SymCryptAesBlockCipher, ShiftLength, &expandedKey, chainingValue, (PCBYTE)Input, (PBYTE)Output, DataLength);
    else
        SymCryptCfbDecrypt(SymCryptAesBlockCipher, ShiftLength, &expandedKey, chainingValue, (PCBYTE)Input, (PBYTE)Output, DataLength);
    ...
    )
{
    ...
    if (error == SYMCRYPT_NO_ERROR)
        SymCryptXtsAesEncryptWith128bTweak(&expandedKey, DataUnitLength, (PCBYTE)Tweak, (PCBYTE)Plaintext, (PBYTE)Ciphertext, DataLength);
    ...
    )
{
    ...
    *CiphertextLength = 0;
    ...
    case 0:
        ...
        error = SymCryptRsaRawEncrypt(key, (PCBYTE)Plaintext, PlaintextLength, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, 0, (PBYTE)Ciphertext, modulusLength);
        if (error == SYMCRYPT_NO_ERROR)
            *CiphertextLength = modulusLength;
        break;
    case 1:
        error = SymCryptRsaPkcs1Encrypt(key, (PCBYTE)Plaintext, PlaintextLength, 0, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, (PBYTE)Ciphertext, CiphertextCapacity, CiphertextLength);
        break;
    case 2:
        ...
        if (!hashAlgorithm)
            ...
            error = SymCryptRsaOaepEncrypt(key, (PCBYTE)Plaintext, PlaintextLength, hashAlgorithm, (PCBYTE)Label, LabelLength, 0, SYMCRYPT_NUMBER_FORMAT_MSB_FIRST, (PBYTE)Ciphertext, CiphertextCapacity, CiphertextLength);
        break;
    default:
        error = SYMCRYPT_INVALID_ARGUMENT;
        break;
