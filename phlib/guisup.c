...
static LRESULT CALLBACK PhWindowSubclassCallback(
    _In_ HWND WindowHandle,
    _In_ UINT uMsg,
    _In_ WPARAM wParam,
    _In_ LPARAM lParam
...
{
    PPH_WINDOW_SUBCLASS context;
    PPH_WINDOW_SUBCLASS_ENTRY entry;
    ULONG i;

    if (!(context = PhGetWindowContext(WindowHandle, PH_WINDOW_SUBCLASS_CONTEXT)))
        return 0;

    for (i = 0; i < context->WindowCallbackArray.Count; i++)
    {
        entry = PhItemArray(&context->WindowCallbackArray, i);

        if (entry->SubclassCallback(WindowHandle, uMsg, wParam, lParam, entry->Context))
        {
...
VOID PhRegisterWindowSubclass(
    _In_ HWND WindowHandle,
    _In_ PPH_WINDOW_SUBCLASS_CALLBACK SubclassCallback,
    _In_opt_ PVOID Context
    )
{
    PPH_WINDOW_SUBCLASS context;
    PH_WINDOW_SUBCLASS_ENTRY entry;

    entry.SubclassCallback = SubclassCallback;
    entry.Context = Context;

    if (context = PhGetWindowContext(WindowHandle, PH_WINDOW_SUBCLASS_CONTEXT))
    {
        PhAddItemArray(&context->WindowCallbackArray, &entry);
    }
    else
    {
        context = PhAllocate(sizeof(PH_WINDOW_SUBCLASS));
        memset(context, 0, sizeof(PH_WINDOW_SUBCLASS));

        PhInitializeArray(&context->WindowCallbackArray, sizeof(PH_WINDOW_SUBCLASS_ENTRY), 1);
        PhAddItemArray(&context->WindowCallbackArray, &entry);

        context->DefaultWindowProc = (WNDPROC)GetWindowLongPtr(WindowHandle, GWLP_WNDPROC);

        PhSetWindowContext(WindowHandle, PH_WINDOW_SUBCLASS_CONTEXT, context);

        SetWindowLongPtr(WindowHandle, GWLP_WNDPROC, (LONG_PTR)PhWindowSubclassCallback);
    }
}

VOID PhUnregisterWindowSubclass(
    _In_ HWND WindowHandle,
    _In_ PPH_WINDOW_SUBCLASS_CALLBACK SubclassCallback
    )
{
