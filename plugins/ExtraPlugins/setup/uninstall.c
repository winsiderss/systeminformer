...
HRESULT CALLBACK TaskDialogUninstallWithoutPromptCallback(
    _In_ HWND hwndDlg,
    _In_ UINT uMsg,
    _In_ WPARAM wParam,
    _In_ LPARAM lParam,
    ...
    )
{
    ...
    config.cxWidth = 200;
    config.pszWindowTitle = L"Process Hacker - Plugin Manager";
    config.pszMainInstruction = PhaFormatString(L"Uninstalling %s...", PhIsNullOrEmptyString(Context->Node->Name) ? PhGetStringOrEmpty(Context->Node->InternalName) : PhGetStringOrEmpty(Context->Node->Name))->Buffer;
    config.dwFlags = TDF_USE_HICON_MAIN | TDF_CAN_BE_MINIMIZED | TDF_SHOW_MARQUEE_PROGRESS_BAR;
    config.dwCommonButtons = TDCBF_CLOSE_BUTTON;
    config.hMainIcon = context->IconLargeHandle;
    config.pfCallback = TaskDialogUninstallWithoutPromptCallback;
    config.lpCallbackData = (LONG_PTR)Context;
    config.pButtons = TaskDialogButtonArray;
    config.cButtons = ARRAYSIZE(TaskDialogButtonArray);
