...
#else
static FILE* FOPEN_FUNC(const wchar_t* filename, wchar_t* mode) 
{ 
    ...
    )
{
    FILE* ftestexist;
    int ret = 1;
    ftestexist = FOPEN_FUNC(filename, TEXT("rb"));
    if (ftestexist==NULL)
        ret = 0;
    else
        fclose(ftestexist);
...
int getFileCrc(const wchar_t* filenameinzip, void*buf,unsigned long size_buf,unsigned long* result_crc)
{
   ...
   int err=ZIP_OK;
   FILE* fin = FOPEN_FUNC(filenameinzip, TEXT("rb"));
...
int isLargeFile(const wchar_t* filename)
{
    ...
    FILE* pFile = FOPEN_FUNC(filename, TEXT("rb"));
