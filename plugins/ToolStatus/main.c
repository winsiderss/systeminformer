...
typedef enum _PH_TOOLBAR_DISPLAY_STYLE
{
    ImageOnly = 0,
    SelectiveText = 1,
    AllText = 2
} PH_TOOLBAR_DISPLAY_STYLE;
...
ULONG StatusMask;
PH_TOOLBAR_DISPLAY_STYLE DisplayStyle;
...
{
	static TBBUTTON buttons[9];
	ULONG buttonIndex;
	ULONG idIndex;
	ULONG imageIndex;

	IdRangeBase = PhPluginReserveIds(9);
...
switch (uMsg)
    ...
    case WM_INITDIALOG:
        {	
			...
			SetDlgItemText(hwndDlg, IDC_DISPLAYSTYLETEXT, L"Toolbar Display Style:");
	...
	case WM_COMMAND:
		{
			...
			case IDOK:
				{
					INT displayStyleSelect = ComboBox_GetCurSel(GetDlgItem(hwndDlg, IDC_DISPLAYSTYLECOMBO));

					if (DisplayStyle != displayStyleSelect)
						PhShowInformation(hwndDlg, L"Restart Process Hacker for Toolbar changes to take effect.");

					PhSetIntegerSetting(L"ProcessHacker.ToolStatus.ToolbarDisplayStyle", 
						(DisplayStyle = displayStyleSelect));
					...
					//Refresh the layout.
					// TODO: reinitilize ToolBar buttons and load settings without requiring PH restart.
					//LoadToolbarSettings();
