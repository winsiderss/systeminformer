...
_Function_class_(DEVPROP_FILL_CALLBACK)
VOID NTAPI DevPropFillTimeStamp(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ const DEVPROPKEY* PropertyKey,
    _Out_ PDEVNODE_PROP Property,
...
_Function_class_(DEVPROP_FILL_CALLBACK)
VOID NTAPI DevPropFillStringList(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ const DEVPROPKEY* PropertyKey,
    _Out_ PDEVNODE_PROP Property,
...
_Function_class_(DEVPROP_FILL_CALLBACK)
VOID NTAPI DevPropFillStringOrStringList(
    _In_ HDEVINFO DeviceInfoSet,
    _In_ PSP_DEVINFO_DATA DeviceInfoData,
    _In_ const DEVPROPKEY* PropertyKey,
    _Out_ PDEVNODE_PROP Property,
...
DEVNODE_PROP_TABLE_ENTRY DevNodePropTable[] =
{
...
ULONG DevNodePropTableCount = RTL_NUMBER_OF(DevNodePropTable);
    ...
    )
{
    ...
    if (node->IconIndex == ULONG_MAX)
    {
        ...
        if (Options->ImageList && SetupDiLoadDeviceIcon(
            node->DeviceInfoHandle,
            DeviceInfoData,
            Options->IconSize.X,
            Options->IconSize.Y,
        ...
        {
            node->IconIndex = PhImageListAddIcon(Options->ImageList, iconHandle);
    ...
    if (Options->FilterSupport && Options->FilterSupport->NodeList)
        node->Node.Visible = PhApplyTreeNewFiltersToNode(Options->FilterSupport, &node->Node);
    else
    ...
    )
{
    ...
    for (ULONG i = 0;; i++)
    {
        ...
        if (!Options->IncludeSoftwareComponents)
        {
            if (IsEqualGUID(&deviceInfoData.ClassGuid, &GUID_DEVCLASS_SOFTWARECOMPONENT))
                continue;
        }

        AddDeviceNode(tree, &deviceInfoData, Options);
    }
