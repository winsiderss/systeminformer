...
현재 커널 드라이버는 필터 포트(Filter Communication Port) 기반 통신 모델을 사용합니다.
기존 `DeviceIoControl` 방식에서 `FltCreateCommunicationPort` 기반으로 전환되었습니다.

```
User Mode                              Kernel Mode
┌──────────────────┐                  ┌──────────────────────┐
│  phlib/kphcomms.c│  Filter Port     │  KSystemInformer.sys │
│                  │◄────────────────►│                      │
│  FltConnectPort()│  FltSendMessage  │  FltCreateComm...()  │
│                  │                  │                      │
│  Ring Buffer     │◄─── 비동기 알림 ──│  Ring Buffer         │
└──────────────────┘                  └──────────────────────┘
```
...
드라이버는 필터 포트 콜백을 통해 클라이언트 연결을 관리합니다:

- `KphpCommsConnectNotifyCallback` - 클라이언트 연결 처리
- `KphpCommsDisconnectNotifyCallback` - 클라이언트 연결 해제 처리
- `KphpCommsMessageNotifyCallback` - 수신 메시지 처리

주요 특징:
- 최대 32개 클라이언트 동시 연결 지원
- 클라이언트 상태 검증 및 권한 확인 (SeDebugPrivilege)
- 프로세서 수에 따라 2~64개 워커 스레드 동적 할당
- 메시지 병합(coalescing)을 통한 동일 클라이언트 메시지 최적화

### 2.3 링 버퍼 기반 비동기 알림

**파일**: `KSystemInformer/ringbuff.c`, `kphlib/include/kphringbuff.h`

커널에서 사용자 모드로의 비동기 이벤트 전달에 링 버퍼를 사용합니다.

```c
// 링 버퍼 주요 API
KphReserveRingBuffer()  // 쓰기 공간 예약
KphCommitRingBuffer()   // 데이터 커밋
KphDiscardRingBuffer()  // 예약 취소
```

- 16바이트 정렬 엔트리로 구성된 원형 생산자-소비자 버퍼
- 자동 랩어라운드(wrap-around) 감지
- 헤더 인코딩: 30비트 길이 필드 + Busy/Discard/Reset 상태 플래그
- 락프리(lock-free) 소비자 추적, 생산자용 스핀락

### 2.4 속도 제한 (Rate Limiting)

**파일**: `KSystemInformer/ratelmt.c`

메시지 타입별 토큰 버킷(token bucket) 기반 속도 제한을 구현합니다.

```c
KphInitializeRateLimit()    // 정책으로 초기화
KphRateLimitConsumeToken()  // 토큰 소비, 허용 여부 반환
```

- 100ms 시간 단위
- 메시지 타입별 독립 정책 지원
- 원자적(atomic) 비교-교환 연산으로 스레드 안전 보장
- 허용/차단/CAS-미스 통계 추적

### 2.5 사용자 모드 메모리 접근

**파일**: `KSystemInformer/umaccess.c`

커널/사용자 경계를 넘는 안전한 메모리 연산을 제공합니다.

```c
KphCaptureUnicodeString()  // 사용자 모드에서 유니코드 문자열 안전 복사
KphCopyToMode()            // 커널→사용자 메모리 복사
KphCopyFromMode()          // 사용자→커널 메모리 복사
KphZeroModeMemory()        // 모드 검증 후 메모리 초기화
```
...
## 13. 고해상도 공급자 (High-Resolution Provider)

### 13.1 개요

**파일**: `phlib/provider.c`, `phlib/include/provider.h`

공급자 스레드의 타이머 정밀도를 높여 저지연 모니터링을 가능하게 합니다.

### 13.2 구현

- `PhSetHighResolutionProvider()` API를 통해 고해상도 모드 활성화/비활성화
- Windows Timer2 API (`NtCreateTimer2`/`NtSetTimer2`) 사용 (가능한 경우)
- Timer2를 사용할 수 없는 환경에서는 표준 타이머로 폴백
- 설정값: `EnableHighResolutionProviderTimer`으로 제어
- 모든 공급자 스레드에 일괄 적용

### 13.3 효과

- 공급자 스레드 웨이크 지연 시간 감소
- 최적화된 웨이크 패턴으로 전력 소비 개선
...
설정 파일 형식을 자동으로 감지하고 최적의 저장소를 선택합니다.
...
- `PhpDiscoverSettingsStores()` - 기존 설정 파일 탐색
- `PhpSelectBestSettingsStore()` - 최고 우선순위 형식 선택
- 설정 파일이 없으면 JSON 형식으로 새로 생성
- 포터블 모드 자동 감지 지원
- 레거시 형식과의 하위 호환성 유지
...
**파일**: `SystemInformer/memprv.c`

프로세스별 데스크톱 힙 할당을 식별하고 추적합니다.

### 15.2 구현

- `DesktopHeapRegion` 메모리 영역 타입 추가
- TEB → Win32ClientInfo → DesktopBase 포인터를 통해 데스크톱 힙 시작 주소 읽기
- 메모리 목록 뷰에 "Desktop heap" 레이블 표시
- 스레드별 데스크톱 힙 추적

### 15.3 의미

- GUI 관련 메모리 오버헤드 프로세스별 식별 가능
- Win32 서브시스템 메모리 할당에 대한 상세 분석 지원
...
| 필터 포트 통신 | 커널-유저 모드 통신 | `KSystemInformer/comms.c` |
| 링 버퍼 | 비동기 이벤트 전달 | `KSystemInformer/ringbuff.c` |
| 속도 제한 | 메시지 흐름 제어 | `KSystemInformer/ratelmt.c` |
| 사용자 모드 접근 | 안전한 메모리 연산 | `KSystemInformer/umaccess.c` |
...
| 고해상도 공급자 | 저지연 모니터링 | `phlib/provider.c` |
| 설정 자동 감지 | 형식 자동 선택 | `phlib/settings.c` |
| 데스크톱 힙 감지 | GUI 메모리 분석 | `SystemInformer/memprv.c` |
